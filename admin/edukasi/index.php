...
<head>
    ...
    <title>Admin Edukasi - InfoMotive</title>
    <!-- Use main style -->
    ...
    <style>
        ...
        .search-form {
            display: contents; /* Ensure form doesn't break flex layout */
        }
        ...
        .filter-select {
            ...
            cursor: pointer;
            outline: none;
            /* Appearance override to match design */
...
<body>
        ...
        <form method="GET" class="controls-row">
            <div class="left-controls">
                ...
                <select name="category" class="filter-select" onchange="this.form.submit()">
                    ...
                    <option value="Tips & Trik" <?= $cat_filter == 'Tips & Trik' ? 'selected' : '' ?>>Tips & Trik</option>
                    <option value="Maintenance" <?= $cat_filter == 'Maintenance' ? 'selected' : '' ?>>Maintenance</option>
                    <option value="Safety" <?= $cat_filter == 'Safety' ? 'selected' : '' ?>>Safety</option>
                    <option value="News" <?= $cat_filter == 'News' ? 'selected' : '' ?>>News</option>
                </select>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 40px;">Data tidak ditemukan.</td>
                        </tr>
                        <?php else: ?>
                            <?php foreach($articles as $i => $a): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td style="font-weight: 500; color: #333;"><?= htmlspecialchars($a['title']) ?></td>
                                <td><?= htmlspecialchars($a['category']) ?></td>
                                <td><?= htmlspecialchars($a['vehicle_type']) ?></td>
                                <td>
                                    <!-- View (Blue) -->
                                    <a href="view.php?id=<?= $a['id'] ?>" class="action-btn btn-view"><i class="fa-solid fa-eye"></i></a>
                                    <!-- Edit (Green) -->
                                    <a href="edit.php?id=<?= $a['id'] ?>" class="action-btn btn-edit"><i class="fa-solid fa-pen"></i></a>
                                    <!-- Delete (Red) -->
                                    <!-- Delete (Red) -->
                                    <button onclick="openDeleteModal(<?= $a['id'] ?>)" class="action-btn btn-delete" style="border:none; cursor:pointer;"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
    ...
    <style>
        ...
        .btn-cancel {
            background: transparent;
            color: #333;
            font-weight: 400; /* Regular weight for 'Cancle' text usually implies link, but here looks like button */
        }
