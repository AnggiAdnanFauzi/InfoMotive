...
$stmt = $pdo->prepare("SELECT * FROM workshops WHERE $where ORDER BY created_at DESC");
$stmt->execute($params);
...
<head>
    ...
    <title>Admin Bengkel - InfoMotive</title>
    <!-- Use main style -->
...
<body>
    ...
    <nav class="navbar-admin">
        <div class="nav-inner">
            ...
            <a href="index.php" class="active">Bengkel</a>
    ...
    <div class="container">
        <!-- Title -->
         <h1 class="page-title">Bengkel</h1>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="4" style="text-align: center; padding: 40px;">Data tidak ditemukan.</td>
                        </tr>
                        <?php else: ?>
                            <?php foreach($workshops as $i => $w): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td style="font-weight: 500; color: #333;"><?= htmlspecialchars($w['name']) ?></td>
                                <!-- Defaulting to vehicle_type or placeholder if not in DB yet, but design suggests column 'Jenis Kendaraan' -->
                                <td><?= htmlspecialchars($w['vehicle_type'] ?? 'Motor') ?></td>
                                <td>
                                    <!-- View (Blue) -->
                                    <a href="../../bengkel.php" target="_blank" class="action-btn btn-view"><i class="fa-solid fa-eye"></i></a>
                                    <!-- Edit (Green) -->
                                    <a href="edit.php?id=<?= $w['id'] ?>" class="action-btn btn-edit"><i class="fa-solid fa-pen"></i></a>
                                    <!-- Delete (Red) -->
                                    <button onclick="openDeleteModal(<?= $w['id'] ?>)" class="action-btn btn-delete" style="border:none; cursor:pointer;"><i class="fa-solid fa-trash"></i></button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
